...
window.addEventListener('load', function() {
    ...
    document.getElementById('run').addEventListener('click', async () => {
        ...
        Utils.showInfo(`可視範囲を計算中... ${mt.name}`);

        try {
            // 観測地点標高(地表からの高さを加算)
            const totalObsElev = inputs.groundInput + (lastTerrainElevation || 0);
    ...
    document.getElementById('period').addEventListener('click', async () => {
        ...
        if (!inputs.startDateStr) {
            alert(CONSTANTS.UI.ERROR_NO_DATE);
            return;
        }
        
        // 最後にクリックした地点の情報を使用
        if (!window.lastClickedLocation) {
            alert('先に地図上で観測地点をクリックしてください');
            return;
        }
        
        const { lat, lon } = window.lastClickedLocation;
        const totalObsElev = inputs.groundInput + lastTerrainElevation;
        const startDate = new Date(inputs.startDateStr);
        
        // パラメータをオブジェクトとして渡す
        await showPeriodsDialog({
            lat: lat,
            lon: lon,
            obsH: totalObsElev,
            mt: mt,
